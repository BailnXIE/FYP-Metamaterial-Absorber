"""
A module for fitting the complex refractive index profile of ckel over a broad
...
def load_maganese_data(filename: str) -> Tuple[np.ndarray, np.ndarray]:
    """
    Load Manganese refractive index data from CSV file.
...
if __name__ == "__main__":
    # Load Manganese refractive index data
    print("Loading Manganese refractive index data...")
    wavelengths, n_complex = load_maganese_data("Material_Mn.csv")
    ...
    num_lorentzians = 9  # Increased for better fit of metal data

    # Number of times to repeat local optimization with random initial values.
    num_repeat = 120  # Increased for better optimization
    ...
    maganese_eps_fitted = lorentzfunc(ps[idx_opt], freqs_reduced) + eps_inf

    fig, ax = plt.subplots(ncols=2, figsize=(12, 5))

    ax[0].plot(wl_reduced, np.real(eps_reduced) + eps_inf, "bo-", label="actual", markersize=3)
    ax[0].plot(wl_reduced, np.real(maganese_eps_fitted), "ro-", label="fit", markersize=3)
    ...
    ax[1].plot(wl_reduced, np.imag(maganese_eps_fitted), "ro-", label="fit", markersize=3)
    ...
    fig.suptitle(
        f"Manganese: Comparison of Actual Material Data and Fit\n"
        f"using Drude-Lorentzian Susceptibility ({num_lorentzians} terms)"
    )

    fig.subplots_adjust(wspace=0.3)
    fig.savefig("eps_fit_maganese.png", dpi=150, bbox_inches="tight")
    print("Plot saved as 'eps_fit_maganese.png'")

    # Print the final Medium definition for use in Meep
    print("\n" + "="*60)
    print("Meep Medium Definition for Manganese:")
    print("="*60)
    print(f"# Meep Medium Definition for Manganese:")

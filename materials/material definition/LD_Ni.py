"""
A module for fitting the complex refractive index profile of Nickel over a broad
...
def load_nickel_data(filename: str) -> Tuple[np.ndarray, np.ndarray]:
    """
    Load Nickel refractive index data from CSV file.
...
if __name__ == "__main__":
    # Load Nickel refractive index data
    print("Loading Nickel refractive index data...")
    wavelengths, n_complex = load_nickel_data("Material_Ni.csv")
    ...
    eps_inf = 1.1  # For metals, this is typically 1.0
    ...
    nickel_eps_fitted = lorentzfunc(ps[idx_opt], freqs_reduced) + eps_inf

    fig, ax = plt.subplots(ncols=2, figsize=(12, 5))

    ax[0].plot(wl_reduced, np.real(eps_reduced) + eps_inf, "bo-", label="actual", markersize=3)
    ax[0].plot(wl_reduced, np.real(nickel_eps_fitted), "ro-", label="fit", markersize=3)
    ...
    ax[1].plot(wl_reduced, np.imag(nickel_eps_fitted), "ro-", label="fit", markersize=3)
    ...
    fig.suptitle(
        f"Nickel: Comparison of Actual Material Data and Fit\n"
        f"using Drude-Lorentzian Susceptibility ({num_lorentzians} terms)"
    )

    fig.subplots_adjust(wspace=0.3)
    fig.savefig("eps_fit_nickel.png", dpi=150, bbox_inches="tight")
    print("Plot saved as 'eps_fit_nickel.png'")

    # Print the final Medium definition for use in Meep
    print("\n" + "="*60)
    print("Meep Medium Definition for Nickel:")
    print("="*60)
    print(f"# Meep Medium Definition for Nickel:")
